...
export default {
  ...
  methods: {
    ...
    async queryList () { // 查询入库货款总金额列表
      let data = await window.axios.post('/report/queryCheckInAmountOfPurchaseList', {
    ...
    async submitExp () {
      ...
      let data = await window.axios.post('/report/checkInAmountOfPurchaseReport', {
        supplierId: _id,
        checkInTimeStart: _start,
        checkInTimeEnd: _end,
        searchContent: `{"供应商名称": "${this.supplierName}", "入库时间": "${this.timeStr}"}`
      });
      if (data.code === 0) {
