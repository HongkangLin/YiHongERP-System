    ...
    <div class="addRole">
      ...
      <div class="content">
        ...
        <div class="base">
          <el-form ref="formData" :model="formData" label-width="110px">
            <el-form-item label="角色类型：">
              ...
              <el-radio v-model="formData.roleType" v-if="formData.roleType !== '1'" label="2">管理员</el-radio>
              <span class="info">管理员可对员工提的审核内容进行审批</span>
              <br />
            ...
            >
              <el-input maxlength="100" v-model="formData.roleName" placeholder="请输入角色名"></el-input>
            </el-form-item>
        ...
        <div class="roleConfig">
          <div class="selArea">
            <div class="selTitle">
              <el-checkbox v-model="checkAll" @change="selAll" key="main">选择全部</el-checkbox>
            </div>
          </div>
            ...
            <div class="selContent">
              <div v-for="(item, index) in items.funcList" :key="'item' + index">
                <el-checkbox
                  ...
                  v-model="item.check"
                  @change="selSome(idx, index)"
                >{{item.funcName.replace(items.menuName + '-', '')}}</el-checkbox>
              </div>
...
export default {
  ...
  methods: {
    ...
    async queryListByRole() {
      // 编辑权限查询
      let data = await window.axios.get(
        `/func/queryFuncListByRoleId/${this.id}`
      );
      data = data.data;
      this.formData = {
    ...
    selSome(idx, index) {
      ...
      let arr = funcList[index].funcName.split("-");
      if (arr[1] !== "查看") {
        // 当前操作非查看
        let pos = "";
        for (let i = 0, len = funcList.length; i < len; i++) {
          if (funcList[i].funcName.split("-")[1] === "查看") {
            // 查看勾选上
            pos = i;
            break;
          }
      ...
      this.$refs[formName].validate(valid => {
        ...
        if (valid) {
          ...
          window.axios.post(url, param).then(data => {
            ...
            window.axios.get("/menu/menu").then(data => {
              // 更新store权限数据
              ...
              let fn = (array, obj) => {
                for (let index = 0; index < array.length; index++) {
                  ...
                  if (item.childNodeList && item.childNodeList.length > 0)
                    item.childNodeList = fn(item.childNodeList, obj);
                  item.funcList.map(i => {
