  ...
  <div>
    ...
    <div class="addSupplier">
        ...
        <div class="right">
          <el-form ref="form" class="form" :model="form" :rules="rules" label-width="170px">
            <el-form-item label="采购单号：">
              <el-input disabled v-model="form.purchaseId"></el-input>
            </el-form-item>
            <el-form-item label="供应商：" prop="supplierId">
              <el-select filterable v-model="form.supplierId" @change="chanSupplier" placeholder="请选择供应商" disabled>
                <el-option
            ...
            <el-form-item label="供应商编号：">
              <el-input disabled v-model="form.supplierSn" placeholder="选择供应商自动带入"></el-input>
            </el-form-item>
      ...
      <div v-if="active === 2" class="page">
        ...
        <div class="right">
          <el-form ref="info" class="form last" :model="form" :rules="infoRule" label-width="210px">
            <el-form-item label="甲方企业抬头：" prop="companyId">
              <el-select filterable v-model="form.companyId" placeholder="请选择甲方企业抬头，不同的企业对应不同的公章">
                <el-option
            ...
            <el-form-item label="乙方收款账号：" prop="accountNo">
              <el-select filterable v-model="form.accountNo" placeholder="请选择乙方收款账号，可选择私账与公账">
                <el-option
            ...
            <el-form-item label="放款方式备注：" prop="payBak">
              <el-input type="textarea" maxlength="100" :rows="7" v-model="form.payBak" placeholder="示例：自双方签订合同起付乙方30%定金27702元，剩余货款64638元出货前付清；"></el-input>
            </el-form-item>
            <el-form-item label="交货日期：" prop="dueTime">
              <el-input type="textarea" :rows="7" v-model="form.dueTime" maxlength="100" placeholder="请输入交货日期"></el-input>
            </el-form-item>
...
3、15个/套（配件：S挂钩和无痕钉各18个,每个相框四个护角然后过塑，2层气泡袋入黑底白字彩盒,包装时要放产品说明书（甲方提供）；"></el-input>
            ...
            <el-form-item label="合同条款（可自定义编辑）：" prop="contractTerms">
              <el-input type="textarea" :rows="11" v-model="form.contractTerms" placeholder="请输入合同条款"></el-input>
            </el-form-item>
          </el-form>
          <el-divider></el-divider>
          <div class="next">
            <div @click="next(2, 1)" class="pre">上一步，选择产品</div>
            <el-button @click="submit" type="primary" :loading="loading">更新采购单</el-button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getDetail() {
      let data = await window.axios.get(`/purchase/queryPurchaseInfo4Update/${this.$route.query.purchaseId}`);
      data.data.purchaseId = this.$route.query.purchaseId;
      this.form = data.data;
      let res = await window.axios.get('/purchase/queryContractTerms/con.purchase');
      this.form.contractTerms = res.data.contractTerms;
    },
    ...
    //   this.form.contractTerms = data.data.contractTerms;
    // },
    ...
    submit () { // 提交
      ...
      this.$refs['info'].validate((valid) => {
        if (valid) {
          for (let index = 0; index < this.compBList.length; index++) {
            if (this.compBList[index].accountNo === this.form.accountNo) {
              this.form.accountName = this.compBList[index].accountName;
              this.form.accountBankname = this.compBList[index].accountBankname;
              continue;
            }
          }
          ...
          window.axios.post('/purchase/updatePurchaseInfo', this.form).then(data => {
            this.loading = false;
            if (data.code === 0) {
              this.$message({
