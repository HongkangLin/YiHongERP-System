        ...
        <div class="right">
          <el-form ref="form" class="form" :model="form" :rules="rules" label-width="170px">
            ...
            <el-form-item label="清关型号：">
              <el-input maxlength="20" v-model="form.clearanceModel" placeholder="请输入清关型号"></el-input>
            </el-form-item>
...
export default {
  ...
  computed: {
    ...
    packingLengthIn () { // 外箱尺寸英寸长
      return this.form.packingLength ? (this.form.packingLength * 0.393700787402).toFixed(2) : '-';
    },
    packingWideIn () { // 外箱尺寸英寸宽
      return this.form.packingWide ? (this.form.packingWide * 0.393700787402).toFixed(2) : '-';
    },
    packingHighIn () { // 外箱尺寸英寸高
      return this.form.packingHigh ? (this.form.packingHigh * 0.393700787402).toFixed(2) : '-';
    },
    disabled () {
      return !!this.id;
    },
    fba () { // fba费计算
      let length = this.goodsLengthIn === '-' ? 0 : parseFloat(this.goodsLengthIn); // 长Inch
      let wide = this.goodsWideIn === '-' ? 0 : parseFloat(this.goodsWideIn); // 宽Inch
      let high = this.goodsHighIn === '-' ? 0 : parseFloat(this.goodsHighIn); // 高Inch
      // 将长宽高排序
  ...
  methods: {
    ...
    async submit (mode) { // 提交(mode->save:存为草稿)
      ...
      param.goodsLengthInch = this.goodsLengthIn !== '-' ? this.goodsLengthIn : 0; // 包装尺寸-长-英寸
      param.goodsWide = param.goodsWide || 0; // 包装尺寸-宽
      param.goodsWideInch = this.goodsWideIn !== '-' ? this.goodsWideIn : 0; // 包装尺寸-宽-英寸
      param.goodsHigh = param.goodsHigh || 0; // 包装尺寸-高
      param.goodsHighInch = this.goodsHighIn !== '-' ? this.goodsHighIn : 0; // 包装尺寸-高-英寸
      param.goodsWeight = param.goodsWeight || 0; // 产品重量
      param.goodsWeightPound = this.goodsWeightLb !== '-' ? this.goodsWeightLb : 0; // 产品重量（磅）
      param.packingWeight = param.packingWeight || 0; // 整箱重量
      param.packingWeightPound = this.packingWeightLb !== '-' ? this.packingWeightLb : 0; // 整箱重量（磅）
      param.packingQuantity = param.packingQuantity || 0; // 装箱数
      param.packingLength = param.packingLength || 0; // 外箱尺寸-长
      param.packingLengthInch = this.packingLengthIn !== '-' ? this.packingLengthIn : 0; // 外箱尺寸-长-英寸
      param.packingWide = param.packingWide || 0; // 外箱尺寸-宽
      param.packingWideInch = this.packingWideIn !== '-' ? this.packingWideIn : 0; // 外箱尺寸-宽-英寸
      param.packingHigh = param.packingHigh || 0; // 外箱尺寸-高
      param.packingHighInch = this.packingHighIn !== '-' ? this.packingHighIn : 0; // 外箱尺寸-高-英寸
